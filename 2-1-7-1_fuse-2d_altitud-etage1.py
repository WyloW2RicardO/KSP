...
def PRESSION(altitud): #pa
    ...
    return planet[pression_pa_0] * (1 - altitud * 6.5 / 1000 / planet[temperature])**(5.255) # imajinair
    ...
    if dim > 0 and end_dim > 1 :
        ...
        for i0 in Q :
            fuse1_plan, fuse1_moteur, fuse1_haut, fuse1_kg_vid, fuse1_kg_fuel, fuse1_kg_total, fuse1_cout = dimension_plus(typ, taille, end_dim - 1,
                                                                                                                           ...
                                                                                                                           fuse_kg_total[i0][0],
                                                                                                                           fuse_cout[i0][0])
            # print('fuse_kg_fuel',i0,fuse_kg_fuel, fuse1_kg_fuel)
            fuse_plan[i0]     = fuse1_plan
            print(dim, end_dim, fuse_plan[i0])
...
def trac(nom, X, x_nom, Y, y_nom, Z = None) :
    matplotlib.pyplot.plot(X, Y)
    if Z != None : matplotlib.pyplot.plot(Z, Y, 'r')
...
ligne0_temp, ligne0_altitud, ligne0_angle, ligne1_temp, ligne1_altitud, ligne1_angle = EULER_3d(but, typ, 
                                                                                                    matric_nombre(fuse0_1moteur, plan), 
                                                                                                    matric_nombre(fuse0_2haut, plan), 
                                                                                                    matric_nombre(fuse0_3kg_vid, plan), 
                                                                                                    matric_nombre(fuse0_4kg_fuel, plan))
ligne0_angle *= 180 / math.pi
ligne1_angle *= 180 / math.pi
trac('vole', ligne0_altitud, 'altitud (m)', ligne0_angle, 'angle (deg)')
dv_plan = D_v(matric_nombre(fuse0_2haut,plan), typ, moteur, matric_nombre(fuse0_3kg_vid, plan), matric_nombre(fuse0_5kg_total, plan))
Z = 1
if Z > 0 :
