...
for i0 in range(1,q):
    nombre       = 0
    fuse0_plan   = numpy.insert(fuse0_plan  , len(fuse0_plan)  , [i0]        , 0)
    fuse1_moteur = numpy.insert(fuse1_moteur, len(fuse1_moteur), [-1]        , 0)
    fuse2_kg_vid = numpy.insert(fuse2_kg_vid, len(fuse2_kg_vid), [charg_kg]  , 0)
    fuse5_cout   = numpy.insert(fuse5_cout  , len(fuse5_cout)  , [charg_cout], 0)
    if i0 > 0 :
        nombre            = i0
        fuse1_moteur[i0]  = 3
    ...
    fuse2_kg_vid[i0][0] += nombre * tanks[1][masse]
    fuse5_cout[i0][0]   += nombre * tanks[1][cout] + fuel_c(nombre)
    fuse3_kg_fuel     = numpy.insert(fuse3_kg_fuel , len(fuse3_kg_fuel) , [fuel_kg(nombre)]                     , 0)
    fuse4_kg_total    = numpy.insert(fuse4_kg_total, len(fuse4_kg_total), [fuse2_kg_vid[i0] + fuse3_kg_fuel[i0]], 0)
