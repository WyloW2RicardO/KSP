...
def trac_plan(plan, typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, step = step0):
    ...
    min_2temp, min_3angle, min_4altitud, x, dz, dx, ddz, ddx = EULER_plan(plan, typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, step)
    # print(x)
    kg_total = matric_nombre(fuse_kg_vid, plan) + matric_nombre(fuse_kg_fuel, plan)
    centre_masse_haut = matric_nombre(fuse_haut, plan)
    temp1, angle1, apog, x1, dz1, dx1, ddz1, ddx1 = APOG(kg_total, min_2temp, min_3angle, min_4altitud, x, dz, dx, ddz, ddx, centre_masse_haut, step)
    # print(x1)
...
def dimension_decol(typ, moteur, kg_total,
    ...
    if dim0 == 0 and dim1 == 1 :
        agravit = [a_GRAVIT(typ, int(moteur[i]), 0, kg_total[i]) for i in range(len(moteur))]
        dim0 = dimension(agravit)
    if dim0 == 0 and dim1 > 1 :
        agravit = [dimension_decol(typ, moteur[i], kg_total[i]) for i in range(len(moteur))]
        dim0 = dimension(agravit)
...
def dimension_dv(typ, moteur, kg_vid, kg_total,
    ...
    if dim0 == 0 and dim1 == 1 :
        ...
        for i in range(1, len(moteur)) :
            dv = numpy.append(dv, dv[0] + D_v(0, typ, moteur[i], kg_vid[i], kg_total[i]))
        dim0 = dimension(dv)
    if dim0 == 0 and dim1 > 1 :
        dv = [dimension_dv(typ, moteur[i], kg_vid[i], kg_total[i]) for i in range(len(moteur))]
        dim0 = dimension(dv)
    # print(fuse_dv)
    return dv
...
# def EULER_1d(typ, moteur, kg_fuel, step = step0) : return range(0, int(fuel_temp(kg_fuel, typ, moteur)), step)
def APOG(kg_total, temp, angle, altitud, x, dz, dx, ddz, ddx, centre_masse_haut, step = step0) :
    ...
    while (apog[i] < but and apog[i] > apog[i - 1]) : # or len(altitud) == len(apog)
        ...
        apog = numpy.append(apog, apog[i] + step * dz1[i] + step**2 * ddz1[i + 1] / 2)
        # print("APOG     apog[i+1]   ", apog[i + 1])
        x1   = numpy.append(x1  , x1[i]   + step * dx1[i] + step**2 * ddx1[i + 1] / 2)
...
def EULER_3d(typ, moteur, haut0, kg_vid0, kg_fuel0,
    ...
    while temp[i] < temp_max and altitud[i] < but and apog[-1] < but :
        ...
        altitud  = numpy.append(altitud, altitud[i] + step * dz[i] + step**2 * ddz[i + 1] / 2)
        ...
        if dx[i + 1] > 0 :
            if 180 * abs(math.atan(dz[i + 1] / dx[i + 1]) - angle[i]) / math.pi < angle_max :
                ...
                angle = numpy.append(angle , angle[i] + angle_max * math.pi / 180)
...
def EULER_plan(plan, typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, step = step0) :
    temp, angle, altitud, x, dz, dx, ddz, ddx = EULER_3d(typ,
...
def dimension_altitud(fuse_plan, typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, fuse_cout,
                      step = step0, 
                      fuse_apog = 0) :
    ...
    if dim1 == 0 and dim0 == 1 :
        ...
        for i in range(len(fuse_plan)) :
            if fuse_cout[i] > 0 :
                temp, angle, altitud, x, dz, dx, ddz, ddx = EULER_plan(fuse_plan[i], typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, step)
    ...
    # print(dim1,dim0)
    if dim1 == 0 and dim0 > 1 :
        fuse_apog = [dimension_altitud(fuse_plan[i], typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, fuse_cout[i], step) for i in range(len(fuse_plan))]
        dim1 = dimension(fuse_apog)
    return fuse_apog
...
if Z > 0 :
    ...
    fuse0_9apog    = dimension_altitud(fuse0_0plan, typ, fuse0_1moteur, fuse0_2haut, fuse0_3kg_vid, fuse0_4kg_fuel, fuse0_6cout1)
    fuse0_6cout1   = dimension_test(fuse0_6cout1, fuse0_9apog, but)
    min_0plan, min_1cout = dimension_min(fuse0_0plan, fuse0_6cout1)
    if dimension(min_0plan) == 1 :   
        if len(min_0plan) == dimension(fuse0_1moteur) :
            trac_plan(min_0plan, typ, fuse0_1moteur, fuse0_2haut, fuse0_3kg_vid, fuse0_4kg_fuel)
