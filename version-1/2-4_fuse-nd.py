...
def cout_multipl(nombre, reserv) : return nombre * tanks[reserv][cout] + tanks[reserv][cout] * (tanks[reserv][multipl] + 1 - nombre) / tanks[reserv][multipl] - 4.25
...
def fuel_kg_d(typ, moteur, nombre=1):
    return nombre * (ergol_u_kg(engeins[typ][moteur][ergol_d]) + oxidizer_u_kg(engeins[typ][moteur][oxidizer_d]))
def fuel_temp(typ, moteur, kg_fuel, nombre=1):
    # print(kg_fuel, fuel_kg_d(typ, moteur))
...
def ISP(typ, moteur, altitud):  # s
    return engeins[typ][moteur][isp_vac] + PA_atm(PRESSION(altitud)) * (engeins[typ][moteur][isp_asl] - engeins[typ][moteur][isp_vac])
...
def RPP(typ, moteur, altitud, kg, nombre=1):  # >1
    return POUSSE(typ, moteur, altitud, nombre) / (kg * GRAVIT(altitud))
def a_GRAVIT(typ, moteur, altitud, kg, nombre=1):
    return GRAVIT(altitud) * (RPP(typ, moteur, altitud, kg, nombre) - 1)


"Changement de vitesse d'un vaisseau spatial mesuré en mètres par seconde (m/s)."
def D_v(typ, moteur, altitud, kg_vid, kg_total):
...
def trac_plan(plan, typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, step=step0):
    global min_2temp, min_3angle, min_4altitud
    min_2temp, min_3angle, min_4altitud, x, dz, dx, ddz, ddx = EULER_plan(
        plan, typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, step)
    # print(x)
    kg_total = matric_nombre(fuse_kg_vid, plan) + \
        matric_nombre(fuse_kg_fuel, plan)
    centre_masse_haut = matric_nombre(fuse_haut, plan)
    temp1, angle1, apog, x1, dz1, dx1, ddz1, ddx1 = APOG(
        kg_total, min_2temp, min_3angle, min_4altitud, x, dz, dx, ddz, ddx, centre_masse_haut, step)
    # print(x1)
    min_3angle *= 180 / math.pi
    matplotlib.pyplot.quiver(x1, apog, ddx1, ddz1, color='r')
    matplotlib.pyplot.quiver(x1, apog, dx1, dz1, color='g')
...
# FONCTION
def construc(taille, typ, moteur,
             fuse_typ=[0],
             fuse_moteur=[-1],
             fuse_haut=[deb_haut],
             fuse_kg_vid=[deb_kg],
    ...
    return fuse_plan, fuse_typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, fuse_kg_total, fuse_cout

"rajoute un etage a la fusee"
def dimension_plus(taille, end_dim,
                   typ=0,
                   moteur=reacteur_deb,
                   fuse_plan=0,
                   fuse_typ=0,
    ...
    if dim == 0 and end_dim == 1:
        fuse_plan, fuse_typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, fuse_kg_total, fuse_cout = construc(taille, typ, moteur,
                                                                                                                    [fuse_typ],
                                                                                                                    [fuse_moteur],
                                                                                                                    [fuse_haut],
                                                                                                                    [fuse_kg_vid],
        ...
        dim = dimension(fuse_moteur)
    if dim == 0 and end_dim > 1:
        fuse_plan, fuse_typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, fuse_kg_total, fuse_cout = dimension_plus(taille, end_dim - 1,
                                                                                                                          typ,
                                                                                                                          moteur,
                                                                                                                          fuse_plan,
                                                                                                                          fuse_typ,
    ...
    if dim > 0 and end_dim > 1:
        ...
        for i0 in Q:
            fuse1_plan, fuse1_typ, fuse1_moteur, fuse1_haut, fuse1_kg_vid, fuse1_kg_fuel, fuse1_kg_total, fuse1_cout = dimension_plus(typ, taille, end_dim - 1,
                                                                                                                                      moteur,
                                                                                                                                      fuse_plan[i0][0],
                                                                                                                                      fuse_moteur[i0][0],
                                                                                                                                      fuse_typ[i0][0],
            ...
            if dim < 2:
                fuse_plan[i0] = [numpy.append(
                    fuse_plan[i0], fuse1_plan[i1]) for i1 in range(len(fuse1_plan))]
            else:
...
# CALCULS
def dimension_decol(typ, moteur, kg_total,
                    agravit=0):
    dim0 = dimension(agravit)
    dim1 = dimension(moteur)
    # print(fuse_moteur)
    if dim0 == 0 and dim1 == 1:
        agravit = [a_GRAVIT(typ, int(moteur[i]), 0, kg_total[i])
                   for i in range(len(moteur))]
        dim0 = dimension(agravit)
    if dim0 == 0 and dim1 > 1:
        agravit = [dimension_decol(typ, moteur[i], kg_total[i])
                   for i in range(len(moteur))]
        dim0 = dimension(agravit)
    return agravit  # fuse_rpp,

def dimension_dv(typ, moteur, kg_vid, kg_total,
                 dv=0):
    dim0 = dimension(dv)
    dim1 = dimension(moteur)
    if dim0 == 0 and dim1 == 1:
        # print(0, typ, fuse_moteur[0], fuse_kg_vid[0], fuse_kg_total[0])
        dv = [D_v(typ, moteur[0], 0, kg_vid[0], kg_total[0])]
        for i in range(1, len(moteur)):
            dv = numpy.append(
                dv, dv[0] + D_v(0, typ, moteur[i], kg_vid[i], kg_total[i]))
        dim0 = dimension(dv)
    if dim0 == 0 and dim1 > 1:
        dv = [dimension_dv(typ, moteur[i], kg_vid[i], kg_total[i])
              for i in range(len(moteur))]
        dim0 = dimension(dv)
    # print(fuse_dv)
    return dv
...
def dimension_test(cout0, fuse, but,
    ...
    if dim1 == 0 and dim0 > 1:
        ...
        cout1 = [dimension_test(cout0[i], fuse[i], but)
                 for i in range(len(cout0))]
        dim1 = dimension(cout1)
...
def dimension_min(plan, cout,
    ...
    if dim > 1:
        for i in range(len(cout)):
            min_plan, min_cout = dimension_min(
                plan[i], cout[i], min_cout, min_plan)
    return min_plan, min_cout

def APOG(kg_total, temp, angle, altitud, x, dz, dx, ddz, ddx, centre_masse_haut, step=step0):
    ...
    while (apog[i] < but and apog[i] > apog[i - 1]):  # or len(altitud) == len(apog)
        ...
        if dx1[i + 1] > 0:
            if 180 * abs(math.atan(dz1[i + 1] / dx1[i + 1]) - angle1[i]) / math.pi < angle_max:
                angle1 = numpy.append(
                    angle1, math.atan(dz1[i + 1] / dx1[i + 1]))
            else:
                angle1 = numpy.append(
                    angle1, angle1[i] + angle_max * math.pi / 180)
        else:
            angle1 = numpy.append(angle1, 0)
        i += 1
        # print(apog[i], apog[i - 1], apog[i] > apog[i - 1])
    # temp1  = temp1[  : len(temp1)  - 1]
    # angle1 = angle1[ : len(angle1) - 1]
    # print(apog)
    # apog   = apog[   : len(apog)   - 1]
    # print(x1)
    # x1     = x1[     : len(x1)     - 1]
    # dz1    = dz1[    : len(dz1)    - 1]
    # dx1    = dx1[    : len(dx1)    - 1]
    # ddz1   = ddz1[   : len(ddz1)   - 1]
    # ddx1   = ddx1[   : len(ddx1)   - 1]
    return temp1, angle1, apog, x1, dz1, dx1, ddz1, ddx1

def EULER_3d(typ, moteur, haut0, kg_vid0, kg_fuel0,
             step=step0,
             temp=[0],
             angle=[0],
             altitud=[0],
    ...
    debit = fuel_kg_d(typ, moteur)
    apog = [0]
    i = len(temp) - 1
    temp_max = fuel_temp(typ, moteur, kg_fuel0) + temp[i]
    # print(temp_max)
    while temp[i] < temp_max and altitud[i] < but and apog[-1] < but:
        # print(i)
        temp = numpy.append(temp, temp[i] + step)
        kg_fuel = kg_fuel0 - i * step * debit
        kg_total = kg_vid0 + kg_fuel
        gravit = GRAVIT(altitud[i] - centre_masse_haut)
        resist = 0
        pousse = POUSSE(typ, moteur, altitud[i])
        ...
        if dx[i + 1] > 0:
            if 180 * abs(math.atan(dz[i + 1] / dx[i + 1]) - angle[i]) / math.pi < angle_max:
                angle = numpy.append(angle, math.atan(dz[i + 1] / dx[i + 1]))
            else:
                angle = numpy.append(
                    angle, angle[i] + angle_max * math.pi / 180)
        else:
            angle = numpy.append(angle, 0)
        # print("EULER_3d angle[i]  ",angle[i])
        temp1, angle1, apog, x1, dz1, dx1, ddz1, ddx1 = APOG(
            kg_total, temp, angle, altitud, x, dz, dx, ddz, ddx, centre_masse_haut, step)
        # print("EULER_3d apog[i]   ",apog[i])
        i += 1
        # print(altitud[i])
    return temp, angle, altitud, x, dz, dx, ddz, ddx

def EULER_plan(plan, fuse_typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, step=step0):
    ...
    for i in range(1, len(plan)):
        ...
        if plan1[-i] != 0:
            ...
            temp, angle, altitud, x, dz, dx, ddz, ddx = EULER_3d(matric_nombre(fuse_typ, plan),
                                                                 ...
                                                                 step, temp, angle, altitud, x, dz, dx, ddz, ddx)
            return temp, angle, altitud, x, dz, dx, ddz, ddx

def dimension_altitud(fuse_plan, fuse_typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, fuse_cout,
                      step=step0,
                      fuse_apog=0):
    ...
    if dim1 == 0 and dim0 == 1:
        ...
        for i in range(len(fuse_plan)):
            if fuse_cout[i] > 0:
                temp, angle, altitud, x, dz, dx, ddz, ddx = EULER_plan(fuse_plan[i], fuse_typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, step)
    ...
    # print(dim1,dim0)
    if dim1 == 0 and dim0 > 1:
        fuse_apog = [dimension_altitud(fuse_plan[i], fuse_typ, fuse_moteur, fuse_haut, fuse_kg_vid, fuse_kg_fuel, fuse_cout[i], step) for i in range(len(fuse_plan))]
        dim1 = dimension(fuse_apog)
    return fuse_apog
...
fuse_0plan, fuse_1typ, fuse_2moteur, fuse_3haut, fuse_4kg_vid, fuse_5kg_fuel, fuse_6kg_total, fuse_7cout = dimension_plus(
    typ, taille, end_dim)
# trac_plan([16,16], typ, fuse0_1moteur, fuse0_2haut, fuse0_3kg_vid, fuse0_4kg_fuel)
Z = 1
if Z > 0:
    fuse_8agravit = dimension_decol(typ, fuse_2moteur, fuse_6kg_total)
    fuse_9dv = dimension_dv(fuse_1typ, fuse_2moteur, fuse_4kg_vid, fuse_6kg_total)
    fuse_7cout1 = dimension_test(fuse_7cout, fuse_8agravit, 1)
    fuse_7cout1 = dimension_test(fuse_7cout1, fuse_9dv, escap(but))
    fuse_10apog = dimension_altitud(fuse_0plan, fuse_1typ, fuse_2moteur, fuse_3haut, fuse_4kg_vid, fuse_5kg_fuel, fuse_7cout1)
    fuse_7cout1 = dimension_test(fuse_7cout1, fuse_10apog, but)
    min_0plan, min_1cout = dimension_min(fuse_0plan, fuse_7cout1)
    if dimension(min_0plan) == 1:
        if len(min_0plan) == dimension(fuse_2moteur):
            trac_plan(min_0plan, fuse_1typ, fuse_2moteur, fuse_3haut, fuse_4kg_vid, fuse_5kg_fuel)
