...
end_dim = 1 + reacteur_fin - reacteur_deb # + 1 + propulseur_fin - propulseur_deb
step0 = end_dim
angle_max0 = step0  # d°
...
def trac_plan(plan, fuse_typ, fuse_moteur, fuse_nombre, fuse_haut, fuse_kg_vid, fuse_kg_fuel, step=step0, angle_max = angle_max0):
    ...
    temp1, angle1, apog, x1, dz1, dx1, ddz1, ddx1 = APOG(kg_total, min_2temp, min_3angle, min_4altitud, x, dz, dx, ddz, ddx, centre_masse_haut, step, angle_max)
    # print(x1)
...
def dimension_plus(end_dim,
    ...
    if dim == 0 and end_dim == 1:
        ...
        dim = dimension(fuse_moteur)
    if dim == 0 and end_dim > 1:
        fuse_plan, fuse_typ, fuse_moteur, fuse_nombre, fuse_haut, fuse_kg_vid, fuse_kg_fuel, fuse_kg_total, fuse_cout = dimension_plus(end_dim - 1,
                                                                                                                          typ,
                                                                                                                          moteur,
                                                                                                                          fuse_plan,
                                                                                                                          fuse_typ,
    ...
    if dim > 0 and end_dim > 1:
        ...
        if moteur + 1 > reacteur_fin :
            typ = 1
            moteur = propulseur_deb
        else : moteur += 1
        Q = range(len(fuse_moteur))
        for i0 in Q:
            fuse1_plan, fuse1_typ, fuse1_moteur, fuse1_nombre, fuse1_haut, fuse1_kg_vid, fuse1_kg_fuel, fuse1_kg_total, fuse1_cout = dimension_plus(end_dim - 1,
                                                                                                                                      typ,
                                                                                                                                      moteur,
                                                                                                                                      fuse_plan[i0][0],
                                                                                                                                      fuse_typ[i0][0],
...
def dimension_dv(typ, moteur, nombre, kg_vid, kg_total,
                 dv=0):
    dim0 = dimension(dv)
    dim1 = dimension(moteur)
    if dim0 == 0 and dim1 == 1:
        # print(0, typ, fuse_moteur[0], fuse_kg_vid[0], fuse_kg_total[0])
        dv = [D_v(typ[0], moteur[0], nombre[0], 0, kg_vid[0], kg_total[0])]
        for i in range(1, len(moteur)):
            dv = numpy.append(dv, dv[0] + D_v(typ[i], moteur[i], nombre[i], 0, kg_vid[i], kg_total[i]))
        dim0 = dimension(dv)
    if dim0 == 0 and dim1 > 1:
        dv = [dimension_dv(typ[i], moteur[i], nombre[i], kg_vid[i], kg_total[i]) for i in range(len(moteur))]
        dim0 = dimension(dv)
    # print(fuse_dv)
    return dv
...
Z = 0
if Z > 0:
    fuse_0plan, fuse_1typ, fuse_2moteur, fuse_3nombre, fuse_4haut, fuse_5kg_vid, fuse_6kg_fuel, fuse_7kg_total, fuse_8cout = dimension_plus(end_dim)
    print('Z', Z - 1)
if Z > 1 :
    ...
    fuse_10dv   = dimension_dv(fuse_1typ, fuse_2moteur, fuse_3nombre, fuse_5kg_vid, fuse_7kg_total)
    fuse_8cout1 = dimension_test(fuse_8cout1, fuse_10dv, escap(but))
    print('Z', Z - 2)
if Z > 2 :
...
if Z > 3 :
    if dimension(min_0plan) == 1:
        if len(min_0plan) == dimension(fuse_2moteur):
            trac_plan(min_0plan, fuse_1typ, fuse_2moteur, fuse_3nombre, fuse_4haut, fuse_5kg_vid, fuse_6kg_fuel, 1, 1)
    print('Z', Z - 4)
