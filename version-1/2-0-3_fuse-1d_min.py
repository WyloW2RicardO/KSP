...
if Z > 0 :
    ...
    fuse0_8deltav  = []
    for i0 in range(q):
        fuse0_6rpp     = numpy.append(fuse0_6rpp    , RPP(fuse0_4kg_total[i0],typ,int(fuse0_1moteur[i0])                 ,0))
        fuse0_7agravit = numpy.append(fuse0_7agravit, a_GRAVIT(fuse0_4kg_total[i0],typ,int(fuse0_1moteur[i0])            ,0))
        fuse0_8deltav  = numpy.append(fuse0_8deltav , D_v(fuse0_2kg_vid[i0],fuse0_3kg_fuel[i0],typ,int(fuse0_1moteur[i0]),0))

    min0_dv = escap(70000)
    for i0 in range(q):
        if fuse0_6rpp[i0] <1 or fuse0_7agravit[i0] < 1 or fuse0_8deltav[i0] < min0_dv:
            fuse0_5cout[i0] = 0
    ...
    for i0 in range(q):
        if fuse0_5cout[i0] != 0:
            if min_1cout == 0:
                min_0plan = fuse0_0plan[i0]
                min_1cout = fuse0_5cout[i0]
            elif fuse0_5cout[i0] < min_1cout:
                min_0plan = fuse0_0plan[i0]
                min_1cout = fuse0_5cout[i0]
            elif fuse0_5cout[i0] == min_1cout :
                min_0plan = fuse0_0plan[i0]
