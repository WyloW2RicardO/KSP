...
# def Dv_altitud(dv, kg_vid, kg_total, typ, moteur, altitud = 0) :
#     #(-altitud / (planet[rayon] + altitud)**2) = (R * planet[temperature]) * math.log(101325 * (dv / GRAVIT(0) / math.log((kg_vid + kg_fuel)/ kg_vid) - engeins[typ][moteur][isp_vac]) / (engeins[typ][moteur][isp_asl] - engeins[typ][moteur][isp_vac]) / planet[pression_pa_0]) / planet[masse_moyen_mol] / G / planet[masse]
#     deltav = D_v(kg_vid, kg_total, typ, moteur, altitud)
#     # if deltav > dv :
#     #     altitud = Dv_altitud(dv, kg_vid, kg_total, typ, moteur, altitud - deltav / dv)
#     # if deltav < dv :
#     #     altitud = Dv_altitud(dv, kg_vid, kg_total, typ, moteur, altitud + dv / deltav)
...
def dimension_rpp(typ, fuse_moteur, fuse_kg_total, fuse_rpp = 0) :
    ...
    # print(fuse_rpp)
    if dim0 == 0 and dim1 > 1 :
        fuse_rpp = [dimension_rpp(typ, fuse_moteur[i], fuse_kg_total[i], fuse_rpp) for i in range(len(fuse_moteur))]
        dim0 = dimension(fuse_rpp)
    # if dim > 1 and dim1 > 1 :
    #     for i0 in range(len(fuse_rpp)) :
    #         fuse_rpp[i0] = [dimension_rpp(typ, fuse_moteur[i1], fuse_kg_total[i1], fuse_rpp[i0]) for i1 in range(len(fuse_moteur))]
    #     dim0 = dimension(fuse_rpp)
    # print(dim0,dim1)
    return fuse_rpp
...
Z = 1
if Z > 0 :
    fuse0_6rpp = dimension_rpp(typ, fuse0_1moteur, fuse0_4kg_total)

    fuse0_6rpp    =[[0 for i1 in range(q)] for i0 in range(q)]
    fuse0_7agravit=[[0 for i1 in range(q)] for i0 in range(q)]
    for i0 in range(q):
        for i1 in range(q):
            fuse0_6rpp[i0][i1]    = RPP(fuse0_4kg_total[i0][i1],typ,fuse0_1moteur[i0][i1],0)
            fuse0_7agravit[i0][i1]= a_GRAVIT(fuse0_4kg_total[i0][i1],typ,fuse0_1moteur[i0][i1],0)


    fuse1_5cout   =[[0 for i1 in range(q)] for i0 in range(q)]
    for i0 in range(q):
        for i1 in range(q):
            if fuse0_7agravit[i0][i1] < 1 or fuse0_6rpp[i0][i1] <1 :
                fuse1_5cout[i0][i1] = 0
            else :
                fuse1_5cout[i0][i1] = fuse0_5cout[i0][i1]

    if Z > 1 :
        
        altitud = 0
        fuse0_8deltav =[[0 for i1 in range(q)] for i0 in range(q)]
        for i0 in range(q):
            i1 = 0
            fuse0_8deltav[i0][i1] = D_v(fuse0_2kg_vid[i0][i1], fuse0_4kg_total[i0][i1], typ, fuse0_1moteur[i0][i1], altitud)
            for i1 in range(1, q):
                fuse0_8deltav[i0][i1] = fuse0_8deltav[i0][0] + D_v(fuse0_2kg_vid[i0][i1], fuse0_4kg_total[i0][i1], typ, fuse0_1moteur[i0][i1], altitud)
        min0_dv = escap(70000)
